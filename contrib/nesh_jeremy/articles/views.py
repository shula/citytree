...
def view(request, slug):
    ...
    return render_to_response('articles/show.html', context_instance=RequestContext(request, {'data': data, 'category': category}))

def kview(request, slug):
    data = get_object_or_404(ArticleCategory, slug__exact=slug)
    return render_to_response('articles/show.html', context_instance=RequestContext(request, {'data': data, 'category': data}))
