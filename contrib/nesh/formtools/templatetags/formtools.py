...
class FormFieldNode(Node):
    ...
    def render(self, context):  #IGNORE:R0912
        ...
        try:
            ...
            # 2part date/time field
            field = None
            field_date = context['FORM_field_date'] = form[self.field + '_date']
            field_time = context['FORM_field_time'] = form[self.field + '_time']
        ...
        try:
            ...
            try:
                ...
                if field is not None:
                    ...
                    if not context['FORM_value']: context['FORM_value'] = ''
                    if context['FORM_value'] == NOT_PROVIDED: context['FORM_value'] = ''
                else:
                    context['FORM_value_date'] = form.data.get(self.field + '_date', 'd?')
                    context['FORM_value_time'] = form.data.get(self.field + '_time', 't?')
...
#            if not self.inline:
#                if field.errors():
#                    ret = '<div class="form-row error">%s</div>' % field
#                else:
#                    ret = '<div class="form-row">%s</div>' % field
#            else:
#                    ret =  str(field)
#            #print repr(ret)
#            return ret
            if not self.inline:
                ...
                elif field is None and (field_date.errors() or field_time.errors()):
                    return '<div class="form-row error">%s</div>' % render_to_string('widgets/%s.html' % template_name, context_instance=context)
                else:
